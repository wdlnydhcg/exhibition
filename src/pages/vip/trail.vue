 ...
 * @Last Modified time: 2018-08-21 15:05:50
 */
<template>
<div style="position:relative;top:0;left:0" id="wrap_all">
    <!-- 轨迹中的移动图片 -->
    <div :class="{move_wrap_img_div:true,
      animationImgShow:animationImgShow,
      moveImgShow:moveImgShow,
      moveImgHide:moveImgHide}" :id="canvas_img_div">
    ...
    <div v-show="currentDialogImg.isShow" :class="{
     ...
     :id="dialog_img_div" :style="{left:currentDialogImg.dialog_img_line_left+'px', top:currentDialogImg.dialog_img_line_top+'px'}">
      <div class="dialog_img_main">
       ...
       <div class="dialog_img_text">
         ...
         <div class="dialog_age"><span>客户年龄段: </span>{{currentDialogImg.dialog_age}}</div>
         <div class="dialog_sex"><span>客户性别: </span>{{currentDialogImg.dialog_sex}}</div>
         <!-- <div class="dialog_name">{{currentDialogImg.dialog_name}}</div>
         <div class="dialog_level">{{currentDialogImg.dialog_level}}</div>
         <div class="dialog_business">{{currentDialogImg.dialog_business}}</div> -->
       </div>
      </div>
     ...
     <canvas id="canvas" :width="2600" :height="1300"></canvas>
     <template v-for="(item, index) in ipcPoint">
       <!-- 停留点位 -->
        ...
        :id="'canvas_ipcPoint_'+index" :key="'ipcPoint-'+index" ></div>
        <!-- 停留时间提示框 -->
        <div :class="{
          time_tip_wrap:true,
        ...
        :id="'canvas_time_tip_'+index" :key="'timeTip-'+index">
...
export default{
  ...
  data () {
    return {
      ...
      moveImgHide: false,
      move_img_src: '01',
      canvas_img_div: 'canvas_img_div', // 移动头像弹框
      dialog_img_div: 'dialog_img_div', // 信息弹框
      canvasId: 'canvas',
  ...
  methods: {
    ...
    // 初始化canvas
    initCanvas (canvasId = this.canvasId, imgDivId = this.canvas_img_div) {
        ...
        if (timerIndex >= allTrailPoints.length) {
          ...
          for (let i = 0; i < currentStayPoint.length; i++) {
            ...
            if (timerIndex === currentStayPoint[i].pointPosition) {
              ...
              _this.currentDialogImg.dialog_img_line_left = allTrailPoints[timerIndex].x - 370 - 8
...
// 设置移动图片退场动画
.moveImgHide{
  animation: toHideAnimation2 2.5s ease-in-out 1;
}
// 设置移动图片的出入动画
.moveImgShow{
